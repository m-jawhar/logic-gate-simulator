...
class Gate(ABC):
    ...
    def get_input_position(self, index: int) -> Tuple[float, float]:
...
class OutputNode:
    ...
    def __init__(self, name: str):
        ...
        self.selected = False
        self.canvas_id: Optional[int] = None
        self.input_wire_id: Optional[int] = None

    def set_value(self, value: Optional[bool]) -> None:
        self.value = value

    def get_input_position(self) -> Tuple[float, float]:
...
class Wire:
    ...
    def __init__(
        self,
        source: Union[Gate, InputNode],
        source_type: Literal["gate", "input"],
        target: Union[Gate, OutputNode],
    ...
    def get_end_pos(self) -> Tuple[float, float]:
        if self.target_type == "output" and isinstance(self.target, OutputNode):
            return self.target.get_input_position()
        if isinstance(self.target, Gate):
            return self.target.get_input_position(self.target_input_index)

        raise ValueError("Invalid wire target configuration")
